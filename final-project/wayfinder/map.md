...
- [Gather and verify the peer-reviewed references](tickets/06-peer-reviewed-references.md) — **Ten references, eight peer-reviewed**, against a requirement of six with three. List at `final-project/resources/references.md`, per-source notes at `final-project/resources/notes/`; every field checked against the source plus two independent records, full text read for all eight articles. Verification changed the plan four times: **Dijkstra (1959) has no complexity analysis at all**, so O(V²) must be cited to Cormen et al.; **Fredman and Tarjan's "lazy deletion" is a different construction** (vacant-node in an F-heap) and is not a citation for this project's duplicate-entry variant; a **DOI-to-venue mismatch** was caught on both methodology sources, which each exist as two registered works; and **Beamer et al. never mention CSR at all**, so Bell and Garland (SC '09) was added to carry Experiment 2's peer-reviewed weight. Best find: **Cormen et al. §22.3, p. 623 states the crossover condition E = o(V²/lg V)**, turning the project's headline result into a published prediction to confirm or refute. The ticket's `Standing cautions` list is a guardrail the writing session must read before drafting.

...
- [Phase 3 report outline — Optimization & Evaluation](tickets/04-phase3-report-outline.md) —
  ...
  References. **Added a 5th source: Google Cloud Pub/Sub docs.** Implementation, demos, and
  benchmark run DONE, and **Phase 3 report WRITTEN + PROOFREAD** at
  `reports/phase3-optimization-and-evaluation.md`: prose proofread to WRITING-REQUIREMENTS and
  aligned with Phases 1-2. Proofread outcomes: Optimizing the Data Structures split into
  **Bounding Memory** + **Restoring Recoverability** `###` subsections (Phase-1-style lead +
  subsections); Opt A/B labels dropped for descriptive names (the memory optimization / the
  recoverability optimization); all phase-name references removed (one cohesive report); FIFO
  abbreviation adopted; `promise` + high/low-water `hysteresis` explained; numbers corrected to
  the figures (Fig 2: N=300-3000, single vs distributed, speedup ~10x→~90x, retry gap; Fig 3:
  11 redeliveries, 8 consumed + 2 dead-lettered = 10; Fig 4: verified); `three metrics`→`three
  dimensions`; failures pinned to `20% of the ordering keys`; strengths/limits/future-work
  closing tightened.
- [Phase 4 assembly + presentation plan](tickets/05-phase4-assembly-presentation.md) —
  Phases 1-3 ASSEMBLED into `reports/final.md` (single APA doc, unified Overview,
  de-duplicated references, Figures 1-4). Review CONFIRMS cohesive/comprehensive (DAG order,
  no quotation marks, all paragraphs ≤300 words). Ticket holds an **assembly/polish
  checklist** (APA number style →numerals ≥10; Hanif ref full proceedings title; stray `\)`;
  **one-sentence lead-in before Figures 2-4**; Fig 4 caption; cite the measured time result in
  Overview; de-duplicate the Component Architecture/Core Data Structures seam; soften the
  Next-Steps→Optimizations transition; `NodeJs`/`en-queue` fixes) and the **presentation
  plan**: brief **HTML deck**, 10 min + live demo; speakers — Monalisa (intro), Parthasarathi
  (design), Saketh (complexity), Nguyen (live demo + optimizations); demo = 3-queue
  ordering/concurrency run live with a recorded backup, Figs 3-4 as charts. **Edits + HTML
  build deferred to execution (not applied).**

## Not yet specified

<!-- empty: the way to the destination is clear. All decisions are made; what remains is
     execution (report polish edits + building the HTML deck), which is out of scope. -->
...
- Applying the Phase 4 assembly/polish edits to `reports/final.md` and building the HTML
  presentation deck — execution work; the decisions (checklist + outline) are recorded in
  [Phase 4 assembly + presentation plan](tickets/05-phase4-assembly-presentation.md).
